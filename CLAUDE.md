...
**Capability gating.** Scarf detects the target's Hermes version once per server connection via [HermesCapabilities](scarf/Packages/ScarfCore/Sources/ScarfCore/Services/HermesCapabilities.swift) (`hermes --version` → semver + `YYYY.M.D` parse). The resulting `HermesCapabilitiesStore` is injected on `ContextBoundRoot` (Mac) and `ScarfGoTabRoot` (iOS) via `.environment(_:)` and `.hermesCapabilities(_:)`; UI that depends on a release-gated surface reads it through the typed environment key. Pre-target hosts gracefully hide the new affordances rather than throwing on unknown CLI subcommands. Add a new flag at the top of `HermesCapabilities` whenever Scarf gains a release-gated UI surface — group flags by the Hermes release that introduced them (`MARK: v0.14 (v2026.5.16) flags`, `MARK: v0.15 (v2026.5.28) flags`, etc.).

**v2026.5.28 (v0.15.0)** — the Velocity Release. Scarf adopts the GUI-relevant subset across three tiers (the bulk of v0.15 — the `run_agent.py` refactor, cold-start perf, promptware defense, `session_search` rebuild, Ink TUI orchestrator, web dashboard, Docker s6, API-server session-control REST — is server-side / other-frontend and benefits Scarf transparently with no code). All surfaces below are capability-gated; pre-v0.15 hosts are unchanged. Exact flag/config/wire shapes were verified against the `v2026.5.28` source before implementation. Commits `e980b65` (Tier 1+2) + `d895136` (Tier 3).

**Kanban — chat `session_id` (slice #1):**

- **Kanban task `session_id` → precise chat-scoped board** ([PR #28447](https://github.com/NousResearch/hermes-agent/pull/28447), salvaging our [#23208](https://github.com/NousResearch/hermes-agent/pull/23208)). Hermes now stamps the originating ACP chat `session_id` on every task created inside the agent loop (the ACP adapter sets `HERMES_SESSION_ID` around `run_conversation`; `kanban_create` reads it — **no agent flag discipline needed**), exposes `session_id` in `hermes kanban list --json`, and accepts `hermes kanban list --session <id>` (ANDs with `--tenant`/`--assignee`). Scarf wires this into `KanbanListFilter.session` → `--session` and `HermesKanbanTask.sessionId` (tolerant decode → nil on older hosts). The chat-header Kanban chip (`SessionInfoBar`) + the chat → board handoff (`KanbanHandoff.sessionId`) are gated on `hasKanbanSessionFilter` (>= 0.15) and filter the board **server-side by session** instead of the old tenant + client-side time-window approximation — which was **removed** (`KanbanBoardViewModel.sessionStartedAt`/`filterBySessionStart`/`applySessionStartFilter` deleted; replaced by `sessionScopeId` + a "This chat" ⇄ "All project tasks" scope toggle). The chip badge (`KanbanChatBadgeViewModel`) also counts session-scoped tasks. The ACP session id Scarf holds in `RichChatViewModel.sessionId` is the *same string* Hermes stamps, so no translation. **The global Kanban sidebar route + per-project Kanban tab stay on `hasKanban` (v0.12+) and tenant-scoped** — only the chat-scoped surface requires v0.15.

**Tier 1 — provider/model catalog + quick wins:**

- **Vercel removed** — Vercel AI Gateway + Sandbox were deleted from Hermes in v0.15. Scarf drops `"vercel"` from `ModelCatalogService.demotedProviders` (now empty), the `vercel/…` entry from `modelAliases`, and `"vercel"` from `SettingsViewModel.terminalBackends`. Unconditional (no flag).
- **OpenAI first-class provider** — wire ID **`openai-api`** (in `HERMES_OVERLAYS`, distinct from `openai-codex`; base URL `https://api.openai.com/v1`, `apiKey` auth). Added to `overlayOnlyProviders`. Note: bare `openai` is a Hermes alias → `openrouter`, so Scarf does **not** register a bare `openai`. Unconditional.
- **Krea image_gen** — `krea-2-medium` / `krea-2-large` added to `ModelCatalogService.imageGenModels`. Unconditional.
- **xAI May-15 retired-model aliases** — `grok-4-0709`, `grok-4-fast-{reasoning,non-reasoning}`, `grok-4-1-fast-{reasoning,non-reasoning}`, `grok-code-fast-1`, `grok-3` → `grok-4.3`; `grok-imagine-image-pro` → `grok-imagine-image-quality` (registered under both `xai` and `xai-oauth` prefixes in `modelAliases`). Mirrors `hermes_cli/xai_retirement.py`. Unconditional. (`qwen3.7-max` needs no action — catalog-driven.)
- **xAI Web Search backend** — wire ID `xai` added to the `WebToolsTab` search-backend list. Gated `hasXAIWebSearchBackend`.
- **ntfy — 23rd messaging platform** — roster entry in `HermesTool.KnownPlatforms` + `NtfySetupView`/`ViewModel`; config under `platforms.ntfy.extra` (`topic` required, `server` default `https://ntfy.sh`, `publish_topic`, `token` password, `markdown` bool); round-trips through both YAML parsers. Gated `hasNtfyPlatform`.
- **Per-platform config flags** — Telegram `disable_topic_auto_rename` (top-level) + `ignore_root_dm`; Discord `allow_any_attachment`; Signal `require_mention` (group-only). Added to the existing setup view-models; ungated (Hermes ignores unknown keys on old hosts).
- **xAI TTS `auto_speech_tags`** — opt-in toggle in `VoiceTab` (config `tts.xai.auto_speech_tags`, default off). Gated `hasXAITTSAutoSpeechTags`.

**Tier 2 — Kanban maturation wave** (single gate `hasKanbanV015`):

- **`list --sort <key>`** — `KanbanListFilter.sort` + a board-header sort picker (keys: created, created-desc, priority, priority-desc, status, assignee, title, updated).
- **New verbs** on `KanbanService`: `promote` (todo/blocked → ready), `schedule` (park into Scheduled), `purge` (`archive --rm`, hard-delete archived), `swarm` (root + workers + verifier + synthesizer). Plus global `--board <slug>` plumbing (prefix on every verb; default nil — board switcher UI deferred).
- **New statuses + columns** — `KanbanStatus` gains `scheduled` + `review`; the board adds **Scheduled** + **Review** columns (collapsed when empty). Card context actions: Promote / Schedule / Delete-permanently (archived, destructive confirm).
- **Task fields** — `HermesKanbanTask` gains `branchName` (`branch_name`), `workflowTemplateId`, `currentStepKey` (all in `list --json`), and `modelOverride` (`model_override`, **show-only** — there's no `--model` create flag; it's set by swarm/decompose internals; surfaced read-only in the inspector alongside a Branch chip). `KanbanCreateRequest` gains `--branch` + a `worktree:<path>` workspace variant.
- The standalone **swarm create-sheet UI is deferred** (service method exists).

**Tier 3 — larger surfaces:**

- **Bitwarden Secrets Manager** — Settings → **Secrets** tab for the `secrets.bitwarden.*` block (`enabled`, `access_token_env` default `BWS_ACCESS_TOKEN`, `project_id`, `override_existing`, `server_url` [empty=US / `vault.bitwarden.eu`=EU / self-hosted], `cache_ttl_seconds` 300, `auto_install` true) + a `hermes secrets bitwarden status` read; round-trips through both parsers. The bootstrap token lives in `~/.hermes/.env`, not config. Gated `hasBitwarden`.
- **Health additions** — a "Run supply-chain audit" button (`hermes audit`, OSV.dev) and an xAI retired-model warning + `hermes migrate xai` action (detects when the configured `xai`/`xai-oauth` model is in the May-15 retired set). Inline-status rendering, mirroring the `--setup-browser` strip. Gated `hasHermesAudit` / `hasXAIModelRetirement`.
- **MCP mTLS** — `client_cert` / `client_key` / `ssl_verify` on HTTP+SSE servers (`HermesMCPServer` fields + parser reads + `setMCPServer{ClientCert,ClientKey,SSLVerify}` writers + an editor "TLS client certificate (mTLS)" section). `ssl_verify` is modeled `String?` (holds `"true"`/`"false"` or a CA path; nil = Hermes default true); `client_cert` list form reads element 0. Plus a read-only `hermes mcp catalog` discovery sheet (text — no `--json`). Gated `hasMCPClientCerts` / `hasMCPCatalog`.
- **Skill bundles** — `HermesSkillBundle` model + `SkillBundlesScanner` reading `~/.hermes/skill-bundles/*.yaml` (`name`/`description`/`skills`/`instruction`) + a read-only **Bundles** tab in `SkillsView`. `HermesPathSet.skillBundlesDir`. Gated `hasSkillBundles`. (Skills Hub freshness badge — index-level only — skipped as low value.)
- **ACP session edit-approval modes** — `ACPClient.setSessionMode(sessionId:modeId:)` wrapping `session/set_mode` + an `ACPApprovalMode` enum (`default` / `accept_edits` / `dont_ask`, advertised in `session/new`'s `modes`) + a chat-header mode chip (`ChatApprovalModeBadge`) with optimistic switch on `RichChatViewModel.activeApprovalMode`. Distinct from the global `approvals.mode`/YOLO surface; sensitive paths always still prompt. Gated `hasSessionEditAutoApproval`.

**Keep in sync on the next Hermes bump:** `overlayOnlyProviders` / `modelAliases` / `demotedProviders` / `imageGenModels` (vs `hermes_cli/providers.py` + `models.py` + `xai_retirement.py`), the platform roster (vs `plugins/platforms/` + `gateway/platforms/`), and the search/TTS backend lists.
...
before starting work, and record durable decisions and learnings *here* (Basic Memory notes,
this block, or the wiki) rather than in any session-private or model-specific memory, so every
session stays consistent and nothing is lost between them.
